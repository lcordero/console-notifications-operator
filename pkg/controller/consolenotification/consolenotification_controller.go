...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &corev1.ConsoleNotification{}}, &handler.EnqueueRequestForOwner{
		IsController: true,
...
func (r *ReconcileConsoleNotification) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	notification := newConsoleNotificationCR(instance)

	// Set ConsoleNotification instance as the owner and controller
	if err := controllerutil.SetControllerReference(instance, notification, r.scheme); err != nil {
		return reconcile.Result{}, err
	}

	// === NEED REVIEW ===
	// Check if this Pod already exists
	// found := &corev1.ConsoleNotification{}
...
// func newPodForCR(cr *operatorv1alpha1.ConsoleNotification) *corev1.Pod {
...
func newConsoleNotificationCR(cr *operatorv1alpha1.ConsoleNotification) *corev1.ConsoleNotification {
	labels := map[string]string{
		"app": cr.Name,
	}
	return &corev1.ConsoleNotification{
		ObjectMeta: metav1.ObjectMeta{
			Name:      cr.Name + "-managed",
			Namespace: cr.Namespace,
			Labels:    labels,
		},
		Spec: corev1.ConsoleNotificationSpec{
			{
				BackgroundColor: cr.BackgroundColor,
				Color:           cr.Color,
				Link:            cr.ConsoleNotificationSpecLink,
